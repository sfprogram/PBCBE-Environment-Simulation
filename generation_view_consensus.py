...
# Change this to load different runs (0, 1, 2, 3, 4)
ENSEMBLE_RUN_ID = 0


with open("training data/consensus/ensemble_params.json") as f:
    params = json.load(f)


csv_files = glob.glob(f"training data/consensus/ensemble_run_{ENSEMBLE_RUN_ID}_seed_*.csv")
if not csv_files:
    raise FileNotFoundError(f"No CSV file found for ensemble run {ENSEMBLE_RUN_ID}")

ensemble_csv_file = csv_files[0]
print(f"Loading from: {ensemble_csv_file}")


ensemble_df = pd.read_csv(ensemble_csv_file)
latest_ensemble_gen = ensemble_df["generation"].max()
latest_consensus_data = ensemble_df[ensemble_df["generation"] == latest_ensemble_gen]


consensus_frog = latest_consensus_data[latest_consensus_data["species"] == "frog"].iloc[0]
consensus_genes = [
    consensus_frog['wall_gene'],
    consensus_frog['food_gene'],
    consensus_frog['snake_gene'],
    consensus_frog['grass_gene'],
    consensus_frog['water_gene']
]
print(f"Using consensus genes from generation {latest_ensemble_gen}: {consensus_genes}")
...
EVOLUTION_STEP_SIZE = params["EVOLUTION_STEP_SIZE"]
MIN_SUCCESSFUL_BEHAVIORS = params["MIN_SUCCESSFUL_BEHAVIORS"]
...
class GeneticSimulation:
    ...
    def load_agents(self):
        for i in range(FROG_POP):

            genome = consensus_genes.copy()
    ...
    def get_best_frog_genome_info(self):
        ...
        genome_str = f"Run {ENSEMBLE_RUN_ID} Gen {latest_ensemble_gen} Genes: Wall:{consensus_genes[0]:.3f} Food:{consensus_genes[1]:.3f} Snake:{consensus_genes[2]:.3f} Grass:{consensus_genes[3]:.3f} Water:{consensus_genes[4]:.3f}"
        return genome_str

    def update_detailed_hud(self):

        hud_lines = []

        hud_lines.append(f"Generation: {latest_ensemble_gen}")
        hud_lines.append(f"Run: {ENSEMBLE_RUN_ID}")
        hud_lines.append("")

        hud_lines.append("FROGS:")
        hud_lines.append("ID  Wall   Food   Snake  Grass  Water")
        hud_lines.append("-" * 40)

        for i, frog in enumerate(self.frogs):
            status = "DEAD" if frog.dead else ("SUCCESS" if frog.succeeded else "ALIVE")
            hud_lines.append(
                f"{i:2d}  {frog.genome[0]:5.2f}  {frog.genome[1]:5.2f}  {frog.genome[2]:5.2f}  {frog.genome[3]:5.2f}  {frog.genome[4]:5.2f}  [{status}]")
        ...
        for i, snake in enumerate(self.snakes):
            ...
            hud_lines.append(
                f"{i:2d}  {snake.genome[0]:5.2f}  {snake.genome[1]:5.2f}  {snake.genome[2]:5.2f}  {snake.genome[3]:5.2f}  {snake.genome[4]:5.2f}  [{status}]")
...
def run_sim():
    root = tk.Tk()
    root.title(f"Consensus Run {ENSEMBLE_RUN_ID} - Generation {latest_ensemble_gen}")
