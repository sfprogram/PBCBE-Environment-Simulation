...
class GeneticSimulation:
    def __init__(self, canvas, data_writer):
        self.canvas = canvas
    ...
    def update(self):
        ...
        if all_dead or self.frog_step > LIFESPAN:
            ...
            if self.frog_generation >= MAX_GENERATIONS:
    ...
    def update_hud(self):
        ...
        hud_lines.append(f"Generation: {self.frog_generation}")
        hud_lines.append(f"Type: {SELECTION_METHOD}")
        hud_lines.append("")
        ...
        for frog in self.frogs:
            frog.temp_fitness = frog.calculate_fitness(self.water_pools)

        sorted_frogs = sorted(enumerate(self.frogs), key=lambda x: x[1].temp_fitness, reverse=True)
        for i, (frog_id, frog) in enumerate(sorted_frogs):
            status = "DEAD" if frog.dead else ("Success" if frog.succeeded else "Alive")
            hud_lines.append(
                f"{frog_id:2d}  {frog.temp_fitness:7.1f}  {frog.genome[0]:5.2f}  {frog.genome[1]:5.2f}  "
        ...
        for i, (snake_id, snake) in enumerate(sorted_snakes):
            ...
            hud_lines.append(
                f"{snake_id:2d}  {snake.fitness:7.1f}  {snake.genome[0]:5.2f}  {snake.genome[1]:5.2f} "
...
def save_parameters():
    ...
    json_filepath = f"training data/GA/{SELECTION_METHOD}_params_main.json"
    try:
        ...
        import os
        os.makedirs("training data/GA", exist_ok=True)
        with open(json_filepath, "w") as pf:
            json.dump(params, pf, indent=2)
        print(f"Parameters saved to {json_filepath}")
...
def run_sim():
    ...
    filename = f"training data/GA/{SELECTION_METHOD}_agent_data_main.csv"
    try:
    ...
    root = tk.Tk()
    root.title(f"Tournament Generation {SELECTION_METHOD}")
    ...
    sim = GeneticSimulation(canvas, writer)
    ...
    def loop():
        if sim.update():
            ...
            csvfile.close()
            print(f"Training completed! Data saved to {filename}")
